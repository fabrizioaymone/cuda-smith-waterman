...
double get_time_gpu_pure() // function to get the time of day in seconds
{
    struct timeval tv;
...
__device__ int max4_gpu_pure(int n1, int n2, int n3, int n4) {
    int tmp1, tmp2;
    tmp1 = n1 > n2 ? n1 : n2;
    tmp2 = n3 > n4 ? n3 : n4;
...
__device__ void backtrace_gpu_pure(char *simple_rev_cigar, char dir_mat[], int i, int j, int max_cigar_len) {
    int k;
...
__global__ void diagonal_pure_kernel(char *query, char *reference, int *res, char *simple_rev_cigar, char *dir_mat_gpu) {
   ...
   // __syncthreads();
    ...
    for (int diag = 2; diag < (2 * S_LEN + 1); diag++) {
        ...
        if (threadId < length) {
            if (row != 0 && col != 0) {
                ...
                tmp = max4_gpu_pure(prev2[threadId + (length_2 - length) / 2] + comparison,
    ...
    if (threadId == 0)
        backtrace_gpu_pure(&simple_rev_cigar[n * S_LEN * 2], dir_mat, maxi, maxj, S_LEN * 2);
...
int gpu_pure(char **query, char **reference, int **sc_mat, char **dir_mat, int *res, char **simple_rev_cigar) {
    ...
    double start_gpu = get_time_gpu_pure();

    diagonal_pure_kernel<<<gridsize, blocksize>>>(query_gpu, reference_gpu, res_gpu, simple_rev_cigar_gpu, dir_mat_gpu);
    CHECK_KERNELCALL();


    CHECK(cudaDeviceSynchronize());
    double end_gpu = get_time_gpu_pure();
